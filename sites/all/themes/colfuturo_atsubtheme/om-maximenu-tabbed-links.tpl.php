...
$is_active = 'no-active';
if('/'.$path == request_uri() || $path == request_uri()){
    $is_active = 'is-active';
}
?>  

<?php if (!empty($permission)): ?>   
  <li id="om-leaf-<?php print $code . '-' . $key; ?>" class="<?php print $is_active ?> <?php print om_maximenu_link_classes($content, $permission, $count, $total); ?>">   
    <?php //print $om_link; ?>
    <span class="om-link">
        <a href="<?php print $content['path'] == '<front>' ? '/' : $content['path']  ?>">
          <?php print $content['link_title'] ?>
      </a>
    </span>
  </li>
<?php endif; ?>  
